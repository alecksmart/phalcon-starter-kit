 ...
 *
 * @method     ChildUserRoleQuery leftJoinUserUserRole($relationAlias = null) Adds a LEFT JOIN clause to the query using the UserUserRole relation
 * @method     ChildUserRoleQuery rightJoinUserUserRole($relationAlias = null) Adds a RIGHT JOIN clause to the query using the UserUserRole relation
 * @method     ChildUserRoleQuery innerJoinUserUserRole($relationAlias = null) Adds a INNER JOIN clause to the query using the UserUserRole relation
 *
 * @method     ChildUserRoleQuery joinWithUserUserRole($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the UserUserRole relation
 *
 * @method     ChildUserRoleQuery leftJoinWithUserUserRole() Adds a LEFT JOIN clause and with to the query using the UserUserRole relation
 * @method     ChildUserRoleQuery rightJoinWithUserUserRole() Adds a RIGHT JOIN clause and with to the query using the UserUserRole relation
 * @method     ChildUserRoleQuery innerJoinWithUserUserRole() Adds a INNER JOIN clause and with to the query using the UserUserRole relation
 *
 * @method     \Main\Main\UserUserRoleQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
...
abstract class UserRoleQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \Main\Main\Base\UserRoleQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'default', $modelName = '\\Main\\Main\\UserRole', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT id, name FROM user_role WHERE id = :p0';
        try {
    ...
    public function filterByName($name = null, $comparison = null)
    {
        ...
        return $this->addUsingAlias(UserRoleTableMap::COL_NAME, $name, $comparison);
    }

    /**
     * Filter the query by a related \Main\Main\UserUserRole object
     *
     * @param \Main\Main\UserUserRole|ObjectCollection $userUserRole the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return ChildUserRoleQuery The current query, for fluid interface
     */
    public function filterByUserUserRole($userUserRole, $comparison = null)
    {
        if ($userUserRole instanceof \Main\Main\UserUserRole) {
            return $this
                ->addUsingAlias(UserRoleTableMap::COL_ID, $userUserRole->getUserRoleId(), $comparison);
        } elseif ($userUserRole instanceof ObjectCollection) {
            return $this
                ->useUserUserRoleQuery()
                ->filterByPrimaryKeys($userUserRole->getPrimaryKeys())
                ->endUse();
        } else {
            throw new PropelException('filterByUserUserRole() only accepts arguments of type \Main\Main\UserUserRole or Collection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the UserUserRole relation
     *
     ...
     * @return $this|ChildUserRoleQuery The current query, for fluid interface
     */
    public function joinUserUserRole($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('UserUserRole');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'UserUserRole');
        }
    ...
    /**
     * Use the UserUserRole relation UserUserRole object
     *
     * @see useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \Main\Main\UserUserRoleQuery A secondary query class using the current class as primary query
     */
    public function useUserUserRoleQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinUserUserRole($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'UserUserRole', '\Main\Main\UserUserRoleQuery');
    }
